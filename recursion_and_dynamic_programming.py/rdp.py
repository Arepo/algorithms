...
def recursive_step_count(n):
    ...
    if n == 3:
        return 4
    return (
      recursive_step_count(n - 1) +
      recursive_step_count(n - 2) +
      recursive_step_count(n - 3))
...
def multiply(x,y):
    def multiply(x,y,log2_of_y):
        ...
        return (
          multiply(x, y, highest_included_power_of_2) +
          multiply(x, remaining_multiple, 0))
...
def number_distribution(num, length):
    ...
    if length == 0:
        # Should only reach this branch with an original value of 0
        return []
    if length == 1:
        return [[num]]
    distributions = []
